...
import iocCnRaw from '../data/ioc-cn-names.json'
 ...
 * 用户"看不懂是谁"。这里维护一份 学名/英文名 → 中文名 的词典：
 *   · 基础大表来自 IOC 中文名数据集（src/data/ioc-cn-names.json，约 1500 种），键已与
 *     前端 normKey 一致（小写 + 去首尾空格 + 连续空白压单空格），自动注入；
 *   · 手写覆盖层（CURATED_CN，实际 82 条），只收"有把握"的常见种，覆盖于大表之上（手写优先）。
 * 命中就直接给中文名（方便用户认识），未命中则退化为英文名 + 学名（诚实不编造）。
 * ⚠️ 覆盖层不止补大表缺口，还兜底"两数据源分类口径差异"：IOC 15.1 对部分属做了拆分/改名
 *   （如 garrulax→trochalopteron、egretta alba→ardea alba），大表只含新学名，旧拼写（如
 *   egretta alba）仅靠覆盖层命中。两层合起来对"旧拼写 + 新学名"端到端全覆盖、无丢失。
 * 新物种由林知声补进 species.sample.json 后，这里可逐步退役。
 */
/** 手写覆盖层：常见中国鸟类 学名/英文名 → 中文名（优先于 IOC 大表，原样保留、不做任何改动） */
const CURATED_CN: ReadonlyArray<readonly [string, string]> = [
  // 学名 → 中文
...
/** IOC 中文名基础大表（扁平 { normkey: 中文名 } 对象，键已与前端 normKey 一致） */
const IOC_CN: Record<string, string> = iocCnRaw as Record<string, string>

/** 合并：大表打底，手写覆盖层叠加其上（后者在前，Map 构造时覆盖前者，实现"手写优先"） */
const UNCATALOGED_CN: ReadonlyMap<string, string> = new Map<string, string>([
  ...Object.entries(IOC_CN),
  ...CURATED_CN.map(([k, v]) => [normKey(k), v] as [string, string]),
])
