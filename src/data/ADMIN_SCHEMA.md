...
> 适用文件：`src/data/species.sample.json`（53 条）、`src/data/recognition-map.json`
> 本文档中的每一条规则均已用现有 53 条数据反向校验通过（0 违规），可直接实现为校验器。
...
现有数据中实际出现的 9 种取值（全部合规，可作为下拉预设 + 允许自定义补充）：
...
| `国家三有保护动物` | **36** | 麻雀、黑枕黄鹂、珠颈斑鸠、乌鸫、白头鹎、大山雀、灰喜鹊、戴胜、大杜鹃、四声杜鹃、白胸苦恶鸟（11 鸟）+ 中华蟾蜍、中国雨蛙（2 蛙）+ 喜鹊、家燕、普通翠鸟、白鹡鸰、八哥、红嘴蓝鹊、松鸦（批 1 +7 鸟）+ 棕背伯劳、北红尾鸲、暗绿绣眼鸟、黑卷尾、山斑鸠、大嘴乌鸦、灰头鹀、红头长尾山雀、鹊鸲、黑水鸡、红耳鹎（批 2 +11 鸟）+ 饰纹姬蛙、泽陆蛙、斑腿泛树蛙、北方狭口蛙、黑眶蟾蜍（批 3 +5 蛙） |
| `国家三有保护动物（IUCN 近危 NT）` | 1 | 黑斑侧褶蛙 |
| `国家三有保护动物（中国特有种）` | 2 | 黄腹山雀（批 2）、镇海林蛙（批 3） |
| `未列入国家保护名录（常见广布种）` | 7 | 黑蚱蝉、蟪蛄、迷卡斗蟋、纺织娘 + 蒙古寒蝉、鸣鸣蝉、黄脸油葫芦（批 3 +3 虫） |
| `未列入国家保护名录（农业常见害虫）` | 1 | 东方蝼蛄（批 3 新增） |
| `未列入国家保护名录（传统鸣虫）` | 1 | 优雅蝈螽（批 3 新增） |
| **合计** | **53** | — |

> 按**基础级别**归并（忽略括号补充说明）则为：三有 39 / 未列入 9 / 一级 2 / 二级 3。
> 上表按**完整字符串精确计数**，两种口径都对，实现校验器时注意区分。

**类群构成（53 条）**：鸟类 **34** / 蛙类 **10** / 昆虫 **9**。
...
**`image` 字段对全部 53 条恒有值**（派生自 `id`），但 **`public/assets/` 下的 webp 文件只对部分物种存在**。二者不是一回事：
...
**其余 48 条无插画是当前的既定状态，不是数据缺陷**（批 1/批 2/批 3 新增物种默认纯文字卡，前端由 `src/data/species.ts` 的鸟/蛙/虫三态 inline SVG 剪影兜底）**。** 前端不会引用它们的 `image`（因为不在 `ILLUSTRATED_IDS` 里），因此**不会产生 404**。
...
- 共 **60 个键 → 42 个 speciesId**，无悬空映射（每个 value 都能在 `species.sample.json` 中找到）。
- 53 条物种卡中 **42 条有映射键、11 条刻意没有**：批 3 新增的 6 蛙 + 5 虫在 BirdNET V2.4 标签表中**完全不存在对应类别**，建键毫无识别价值，故只做内容卡、不写映射（详见 §6.3）。**校验器不应把这 11 条报为错误**，最多计入「未接入识别链路 N 条」的汇总提示。
...
- ✅ **33 / 53 可被 BirdNET 识别**（除朱鹮外的全部 33 种鸟；批 1 +8 鸟、批 2 +12 鸟，均在 6522 类标签内）
- ❌ **20 / 53 BirdNET 完全无法识别**：
  - `crested-ibis` 朱鹮 *Nipponia nippon* —— **不在** 6522 类中（极危物种，训练数据不足）
  - 蛙类 10 种：黑斑侧褶蛙、中华蟾蜍、中国雨蛙、虎纹蛙 + 饰纹姬蛙、泽陆蛙、斑腿泛树蛙、北方狭口蛙、镇海林蛙、黑眶蟾蜍（批 3）
  - 昆虫 9 种：黑蚱蝉、蟪蛄、迷卡斗蟋、纺织娘 + 蒙古寒蝉、鸣鸣蝉、东方蝼蛄、黄脸油葫芦、优雅蝈螽（批 3）

> **批 3 的定位**：这 11 条是**纯内容卡**，明知识别链路够不到仍然收录——目的是让「听籁」页与自然大厅的科普内容覆盖到夏夜最有存在感的蛙鸣与虫鸣，同时为将来接入自训练分类器预留数据。前端展示、检索、Quiz 均可正常使用，只是不会被 BirdNET 命中。
...
映射表中**原有 8 条蛙类/昆虫**（黑斑侧褶蛙、中华蟾蜍、中国雨蛙、虎纹蛙、黑蚱蝉、蟪蛄、迷卡斗蟋、纺织娘）仍保留正名与同物异名键，是为**将来接入自训练分类器或人工标注**时可直接复用，无需改结构。**批 3 新增的 11 条蛙/虫未建键**，将来同样只需按此结构补键即可，不影响现有数据。
...
> 📌 **已知分类学分歧但未建键**（批 3 物种 BirdNET 无覆盖，建键无意义，仅在此备案）：
> 蒙古寒蝉本卡片用 `Meimuna mongolica`（国内文献通用名），iNaturalist 现行分类已移入 **`Streeyola mongolica`**；
> 泽陆蛙 `Fejervarya multistriata` 另有 *Zakerana / Sphaerotheca multistriata* 等组合名在流通。
> 将来若接入自训练分类器，按 §6.4 结构补键即可。
...
| 数据 | 来源 | 许可 |
|---|---|---|
| 物种叫声音频 | [iNaturalist](https://www.inaturalist.org/)（52 条）、[xeno-canto](https://xeno-canto.org/)（1 条），全部已下载落地 `public/audio/` | 逐条记录于 `audioLicense`，均为 CC0 / CC BY / CC BY-NC / CC BY-NC-SA 4.0 |
